    ...
    if json_data.get("cod") != "404" and json_data.get("cod") != "400":
        dict_info = {
            ...
            "sunset": time.strftime(
                "%I:%M:%S", time.gmtime(json_data["sys"]["sunset"] - 21600)
            ),
            "name": json_data["name"],
...
def main(page: flet.Page):
    ...
    page.theme = flet.theme.Theme(color_scheme_seed="#000000")
    page.bgcolor = "#000000"
    ...
    dark_theme = flet.IconButton(
        icon=flet.icons.NIGHTLIGHT,
        icon_size=20,
        tooltip="dark theme",
        on_click=set_dark_theme,
    )

    theme_change = flet.Row(
        [
            light_theme,
            dark_theme,
        ]
    ...
    temp_values_str = "Temp  : {} °C / {}  °F \n Feels like  :  {} °C / {}  °F".format(
        tmp_tup[0], tmp_tup[1], tmp_tup[2], tmp_tup[3]
    )

    city_weather_temp = flet.Text(
    ...
    weather_desc = flet.Row(
        [city_weather_desc, city_weather_icon, city_weather_temp], alignment="center"
    )

    #  Row_02 -- sun_desc = [sunrise_icon,sunrise_time,sunset_icon,sunset_time].
    sunrise_icon = flet.Image(
    ...
    def search_clicked(city):
        city_weather_icon.src = "http://openweathermap.org/img/wn/{}@2x.png".format(
            "03d"
        )
        ...
        try:
            ...
            if weather_info.get("cod") != "404" and weather_info.get("cod") != "400":
                info.value = "{} {} , {}".format(
                    " " * 10, weather_info["name"], weather_info["country"]
                )
                city_weather_desc.value = weather_info["weather_desc"]
                city_weather_icon.src = (
                    "http://openweathermap.org/img/wn/{}@2x.png".format(
                ...
                sunrise_time.value = "Sunrise   :   {}  AM{}".format(
                    str(weather_info["sunrise"])[:5], " " * 20
                )
                sunset_time.value = "Sunset   :   {}  PM".format(
                    str(weather_info["sunset"])[:5]
                )

                #  Row_03.value.update -- {humidity_details,wind_speed,pressure_details}.value.update.
                humidity_details.value = "Humidity  :   {} %{}".format(
                    weather_info["humidity"], " " * 5
                )
                wind_speed.value = "Wind Speed    :   {} m/s{}".format(
                    weather_info["wind_speed"], " " * 5
                )
                pressure_details.value = "Pressure  :   {} hPa".format(
    ...
    city_name = flet.TextField(
        hint_text="Name of the City",
        icon=flet.icons.LOCATION_ON,
        color="white",
        border_color="white",
